...
AI로 이미지를 만든다. 프롬프트 한 줄 쓰면 그럴듯한 그림이 나온다. 그렇게 들었다.

실제로 해보면 현실은 좀 다르다. "예쁜 여성 캐릭터"를 만들고 싶었을 뿐인데, 나오는 건 누구인지 모를 얼굴에 손가락이 6개고 옷은 녹아내리는 무언가다. 뭐가 잘못된 건지 감도 안 잡힌다.

이 시리즈는 그 삽질의 기록이다. 로컬 환경에서 SD 1.5 모델을 돌리며, 여성 캐릭터 일러스트 하나를 완성하기까지 겪은 과정을 단계별로 정리했다. 화려한 결과물 자랑이 아니라, **뭘 시도했고 뭐가 안 됐고 어떻게 고쳤는지**에 초점을 맞췄다.


## 환경

- **모델**: Stable Diffusion 1.5 (DreamShaper 8)
- **UI**: ComfyUI
...
이 시리즈에서는 **DreamShaper 8**을 메인으로 사용한다. 범용적이라 다양한 실험을 하기에 적합하다.

### 삽질 1: 모델을 아무거나 골랐다

처음에는 "SD 1.5면 다 비슷하겠지"라고 생각했다. 아니었다. 어떤 모델은 얼굴이 뭉개지고, 어떤 모델은 색감이 탁하고, 어떤 모델은 특정 스타일에서만 결과가 좋았다.

**교훈**: 모델은 화구다. 수채화 물감으로 유화를 그릴 수 없듯이, 실사 모델로 애니메이션 스타일을 뽑으면 어정쩡한 결과가 나온다. 원하는 스타일에 맞는 모델을 먼저 골라야 한다.
...
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | CFG 7.0 | Seed 42
...
> **Positive**: young woman, black hair, red dress, garden, sunset lighting
>
> **Negative**: ugly, deformed, bad anatomy, bad hands, missing fingers, extra fingers, blurry, low quality, worst quality, watermark, text, signature
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | CFG 7.0 | Seed 42
...
> **Positive**: (red dress:1.2), young woman, black hair, garden
>
> **Negative**: ugly, deformed, bad anatomy, bad hands, missing fingers, extra fingers, blurry, low quality, worst quality, watermark, text, signature
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | CFG 7.0 | Seed 42
...
> **Positive**: young woman, black hair, red dress, portrait
>
> **Negative**: ugly, deformed, bad anatomy, bad hands, missing fingers, extra fingers, blurry, low quality, worst quality, watermark, text, signature
>
> **설정**: DreamShaper 8 | **1024x1024** | euler_ancestral | Steps 20 | CFG 7.0 | Seed 42
...
**해결**: 512x768로 생성한 다음, 나중에 업스케일한다. 이건 9화에서 다룬다.
...
- **샘플러**: `euler_ancestral`이 무난한 시작점. `dpm++ 2m karras`는 좀 더 선명한 결과를 준다.
...
> **Negative**: ugly, deformed, bad anatomy, bad hands, missing fingers, extra fingers, blurry, low quality, worst quality, watermark, text, signature
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | **CFG 15.0** | Seed 42
...
> **Positive**: young woman, black hair, red dress, garden
>
> **Negative**: *(없음)*
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | CFG 7.0 | Seed 42
...
네거티브 프롬프트는 "이것만 넣으면 만사 해결"이 아니다. 안 넣으면 확률적으로 더 자주 나오는 문제를 줄여줄 뿐이다. 손가락 6개가 나오는 문제는 네거티브로 완전히 해결되지 않는다. 그건 인페인팅으로 잡아야 하고, 4화에서 다룬다.
...
> **Positive**: young woman, black hair, red dress, soft lighting, detailed face
>
...
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | CFG 7.5 | Seed 777

![괜찮은 결과](images/01/08_default_ok.png)

얼굴은 비교적 자연스럽다. 하지만 자세히 보면 AI 특유의 과도한 피부 매끈함이 있다. 실제 사람의 피부 질감과는 거리가 있다.

**손가락이 이상한 결과:**

> **Positive**: young woman, black hair, showing both hands, waving, casual outfit, upper body
>
> **Negative**: ugly, deformed, bad anatomy, bad hands, missing fingers, extra fingers, blurry, low quality, worst quality, watermark, text, signature
>
> **설정**: DreamShaper 8 | 512x768 | euler_ancestral | Steps 20 | CFG 7.0 | Seed 123
...
이 "안 되는 것" 목록이 이 시리즈의 나머지 10화를 채운다.
...
- **가중치는 1.3 이하**로 절제
- **CFG는 7~8**, 샘플러는 euler_ancestral 또는 dpm++ 2m karras
- **시드를 기록**해두면 나중에 재현할 수 있다
- txt2img만으로는 한계가 명확하다. 여기서부터 시작이다.

다음 화에서는 "내가 원하는 포즈가 안 나온다"는 문제를 해결한다. ControlNet이 등장한다.
