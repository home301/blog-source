...
# RTX 3050 6GB로 Stable Diffusion Forge 설치하기 — 2026년 완벽 가이드

> VRAM 6GB짜리 저사양 GPU로도 AI 이미지 생성이 가능할까? 직접 설치하고 돌려본 생생한 후기.

---

## 왜 Forge인가?

AI 이미지 생성의 대명사 **Stable Diffusion**을 로컬에서 돌리려면 WebUI가 필요합니다. 가장 유명한 건 **AUTOMATIC1111**이지만, 2026년 현재 추천하는 건 **Stable Diffusion WebUI Forge**입니다.
...
> 💡 **Windows 사용자도** 과정은 거의 동일합니다. 차이점은 아래에서 별도 안내합니다.
...
Forge는 **Python 3.10**을 요구합니다. 3.11이나 3.12에서는 torch 호환 문제가 발생합니다.
...
> ⚠️ **실제 겪은 문제:** Python 3.12에서 먼저 시도했더니 `torch==2.1.2` 버전을 찾을 수 없다는 에러가 발생했습니다. Python 3.10으로 바꾸니 바로 해결.
...
드라이버 버전이 **530 이상**이면 OK. 안 나오면 NVIDIA 드라이버부터 설치해야 합니다.
...
> 이 단계를 건너뛰면 시스템 Python(3.12 등)이 사용되어 에러 발생 가능.
...
> ⏱️ **소요 시간:** 인터넷 속도에 따라 10~30분. 저는 약 15분 걸렸습니다.

### Step 4: 정상 실행 확인

모든 설치가 끝나면 이런 메시지가 나옵니다:

```
Running on local URL: http://127.0.0.1:7860
```

브라우저에서 `http://localhost:7860` 접속하면 WebUI가 나타납니다!
...
**영향:** ControlNet 일부 프리프로세서만 영향. 핵심 이미지 생성에는 문제없음. 무시해도 됩니다.
...
| 모델 | 해상도 | 생성 가능 여부 | 비고 |
|------|--------|--------------|------|
| **SD 1.5** | 512x512 | ✅ 쾌적 | 가장 추천 |
| **SD 1.5** | 768x768 | ✅ 가능 | 약간 느림 |
| **SDXL** | 1024x1024 | ⚠️ 가능하나 느림 | xformers 필수 |
| **SD 3** | 1024x1024 | ❌ VRAM 부족 | 8GB 이상 권장 |
| **Flux** | — | ❌ VRAM 부족 | 12GB 이상 권장 |

> 💡 **팁:** `--xformers` 옵션은 **필수**입니다. VRAM 사용량을 크게 줄여줍니다. Forge는 여기에 추가 최적화가 적용되어 AUTOMATIC1111보다 약 20~30% 적은 VRAM을 사용합니다.

---

## 모델 다운로드

Forge 설치만으로는 이미지 생성이 안 됩니다. **체크포인트 모델**을 따로 받아야 합니다.
...
RTX 3050 6GB는 AI 이미지 생성의 **최소 스펙**에 가깝지만, Forge + xformers 조합이면 SD 1.5 모델은 충분히 실용적으로 사용 가능합니다.

**총 소요 시간:** 약 30분 (트러블슈팅 포함 1시간)
**총 비용:** 0원 (모든 소프트웨어 무료)

고사양 GPU가 없어도 로컬 AI 이미지 생성을 시작할 수 있다는 게 핵심입니다. 다음 글에서는 실제로 이미지를 생성하고 프롬프트 작성법을 다뤄보겠습니다.
...
*이 글은 실제 설치 과정을 기록한 것입니다. 환경에 따라 에러가 다를 수 있습니다.*
